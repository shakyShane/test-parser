...
const defaults     = {
    ...
    errors: {
        ...
        'NOT_CLOSED': (node, c) => {
            return [
                `------~~-----`,
                `<pre>${node.errorLine()}</pre>`,
                `Tag not closed correctly on LINE: ${node.error.loc.line}`,
                `------~~-----`,
            ].join('\n');
        },
        'CLOSE_TAG_MISSING': (node) => {
            return [
                `------~~-----`,
                `\`${node.value}\` Tag not closed correctly`,
                `------~~-----`,
            ].join('\n');
        }
    }
...
class Compiler {
    ...
    blockVisitor (node, ctx) {
        ...
        if (node.type === 'BLOCK' && node.closed === false) {
            return this.returnError('CLOSE_TAG_MISSING', node);
        }
        if (node.blockType === '#') {
    ...
    nodeError (node) {
        const error = this.returnError(node.error.type, node);
        return `Syntax Error:
    ${error}
`
    }

    returnError (errorName, node) {
        return this.opts.errors[errorName](node, this);
    }
}
